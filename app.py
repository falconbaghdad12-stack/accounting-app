...
def trial_balance_df(date_from=None, date_to=None):
    accounts = get_accounts_df()
    accounts = leaf_accounts_only(accounts)
    rows = []
    for _, acc in accounts.iterrows():
        df = get_ledger_df(acc["code"], date_from, date_to)
        total_debit = df["مدين"].sum()
        total_credit = df["دائن"].sum()
...
def income_statement(date_from=None, date_to=None):
    accounts = get_accounts_df()
    accounts = leaf_accounts_only(accounts)
    revenues = accounts[accounts["type"] == "ايرادات"]
    expenses = accounts[accounts["type"] == "مصروفات"]

    rev_rows, exp_rows = [], []
    total_rev, total_exp = 0, 0
    for _, acc in revenues.iterrows():
        bal = account_balance(acc["code"], acc["nature"], date_from, date_to)
        if bal != 0:
...
def balance_sheet(date_from=None, date_to=None):
    accounts = get_accounts_df()
    accounts = leaf_accounts_only(accounts)
    assets = accounts[accounts["type"] == "اصول"]
    liabilities = accounts[accounts["type"] == "خصوم"]
    equity = accounts[accounts["type"] == "حقوق الملكية"]
    ...
    _, _, _, _, net_income = income_statement(date_from, date_to)
...
if menu == "🏠 الرئيسية":
    ...
    _, total_rev, _, total_exp, net_income = income_statement()
    assets_df, total_assets, liab_df, total_liab, equity_df, total_equity, _ = balance_sheet()
