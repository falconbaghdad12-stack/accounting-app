...
# لما موظف يضغط زر "وصول سريع" بالصفحة الرئيسية، نحفظ الصفحة المطلوبة بالجلسة
# ونستخدمها كقيمة افتراضية لقائمة التنقل الجانبية بدل ما يضطر يدوّر عليها يدويًا
if "quick_nav" in st.session_state:
    _default_menu_index = MENU_OPTIONS.index(st.session_state.pop("quick_nav"))
else:
    _default_menu_index = 0

menu = st.sidebar.radio("القائمة الرئيسية", MENU_OPTIONS, index=_default_menu_index)
...
if menu == "🏠 الرئيسية":
    ...
    def app_tile(col, icon, label, color, target_menu, key):
        with col:
            st.markdown(f"""
            <div style="background:{color}1A; border:1px solid {color}55; border-radius:14px;
                        padding:14px 8px 8px 8px; text-align:center; margin-bottom:4px;">
                <div style="background:{color}; width:52px; height:52px; border-radius:14px;
            ...
            if st.button(label, key=key, use_container_width=True):
                st.session_state["quick_nav"] = target_menu
                st.rerun()
    ...
    with st.container(border=True):
        ...
        s1.metric("💰 الإيرادات", f"{total_rev:,.2f}")
        s2.metric("🧾 المصروفات", f"{total_exp:,.2f}")
        if net_income >= 0:
            s3.metric("📈 صافي الربح", f"{net_income:,.2f}")
        else:
            s3.metric("📉 صافي الخسارة", f"{net_income:,.2f}")
        s4.metric("🏦 صافي حقوق الملكية", f"{total_equity:,.2f}")

    st.divider()
    st.markdown("### ⚡ الوصول السريع")
    st.caption("مرتّبة حسب الاستخدام اليومي للموظف المباشر — الأهم على اليمين")

    r1c1, r1c2 = st.columns(2)
    app_tile(r1c1, "💰", "حركة صندوق (قبض / صرف)", "#00A09D", "💰 حركة صندوق (قبض / صرف)", "qn_cash")
    app_tile(r1c2, "🧾", "تسجيل مصروف", "#E4685D", "🧾 مصروفات", "qn_expense")

    r2c1, r2c2 = st.columns(2)
    app_tile(r2c1, "🧾", "فاتورة مبيعات", "#9C6ADE", "🧾 فاتورة مبيعات", "qn_sales_inv")
    app_tile(r2c2, "📦", "فاتورة مشتريات", "#2E86DE", "🧾 فاتورة مشتريات", "qn_purch_inv")

    r3c1, r3c2 = st.columns(2)
    app_tile(r3c1, "👥", "كشف حساب عميل", "#2ECC71", "👥 العملاء (ذمم مدينة)", "qn_customers")
    app_tile(r3c2, "🏭", "كشف حساب مورد", "#F0AD4E", "🏭 الموردون (ذمم دائنة)", "qn_suppliers")
    ...
    if not df.empty:
        ...
        st.info("لا توجد قيود بعد. ابدأ بتسجيل حركاتك من الأزرار أعلاه أو القائمة الجانبية.")
