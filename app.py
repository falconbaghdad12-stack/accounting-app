...
def post_sales_invoice(invoice_date, customer_id, payment_type, cash_account, tax_id, lines):
    ...
    ok, msg = post_journal_entry(invoice_date, "فاتورة مبيعات", je_lines, source="فاتورة مبيعات")
    if not ok:
        return False, msg
    cur.execute("SELECT id FROM journal ORDER BY id DESC LIMIT 1")
    ...
    cur.execute(
        """INSERT INTO sales_invoices (invoice_date, customer_id, payment_type, cash_account_code,
           tax_id, subtotal, tax_amount, total_amount, journal_id, created_at)
           VALUES (?,?,?,?,?,?,?,?,?,?)""",
        (str(invoice_date), customer_id, payment_type, cash_account if payment_type == "نقدي" else None,
         tax_id, subtotal, tax_amount, total_amount, journal_id, datetime.now().isoformat())
    )
...
def post_purchase_invoice(invoice_date, supplier_id, payment_type, cash_account, tax_id, lines):
    """
    ...
    if je_lines:
        ...
        ok, msg = post_journal_entry(invoice_date, "فاتورة مشتريات", je_lines, source="فاتورة مشتريات")
        if not ok:
            return False, msg
        cur.execute("SELECT id FROM journal ORDER BY id DESC LIMIT 1")
        journal_id = cur.fetchone()[0]
    ...
    cur.execute(
        """INSERT INTO purchase_invoices (invoice_date, supplier_id, payment_type, cash_account_code,
           tax_id, subtotal, tax_amount, total_amount, journal_id, created_at)
           VALUES (?,?,?,?,?,?,?,?,?,?)""",
        (str(invoice_date), supplier_id, payment_type, cash_account if payment_type == "نقدي" else None,
         tax_id, subtotal, tax_amount, total_amount, journal_id, datetime.now().isoformat())
    )
    invoice_id = cur.lastrowid
...
# ---------------------------------------------------------------------------
    ...
    st.subheader("فاتورة مبيعات")
    st.caption("فاتورة متعددة البنود بالكمية ووحدة القياس، تخصم من المخزون وترحّل قيد الإيراد وتكلفة البضاعة المباعة تلقائيًا")
    ...
    if uom_opts_df.empty:
        ...
        if si_sale_type == "آجل (على الحساب)":
            with st.expander("➕ إضافة عميل جديد بسرعة"):
                ...
                nc_phone = nc2.text_input("الهاتف (اختياري)", key="si_new_cust_phone")
                nc_open = nc3.number_input("رصيد افتتاحي", min_value=0.0, step=0.01, key="si_new_cust_open")
        ...
        c1, c2 = st.columns(2)
        si_date = c1.date_input("تاريخ الفاتورة", value=date.today(), key="si_date")
        si_cash_acc = None
        ...
        si_tax_choice = c2.selectbox(
            "الضريبة", tax_choice_options,
            format_func=lambda t: "بدون ضريبة" if t == "بدون ضريبة"
        ...
        edited_si = st.data_editor(
            ...
            column_config={
                "الصنف / الوحدة": st.column_config.SelectboxColumn("الصنف / الوحدة", options=options, required=True),
                "الكمية": st.column_config.NumberColumn("الكمية", min_value=0.0, step=0.01),
                "سعر الوحدة": st.column_config.NumberColumn("سعر الوحدة", min_value=0.0, step=0.01),
            },
            key="si_editor"
        )

        preview_subtotal = float((edited_si["الكمية"] * edited_si["سعر الوحدة"]).sum())
        st.metric("الإجمالي قبل الضريبة (معاينة)", f"{preview_subtotal:,.2f}")

        if st.button("✅ ترحيل فاتورة المبيعات", type="primary"):
            lines = []
            for _, r in edited_si.iterrows():
                ...
                prod_id = int(uom_opts_df[uom_opts_df["uom_id"] == uom_id]["product_id"].values[0])
                lines.append({"product_id": prod_id, "uom_id": uom_id,
...
# ---------------------------------------------------------------------------
    ...
    st.subheader("فاتورة مشتريات")
    st.caption("فاتورة متعددة البنود: بنود مخزون تُحدّث الكمية ومتوسط التكلفة تلقائيًا، وبنود أصول ثابتة "
               "يمكنك تحويلها لاحقًا لموديل الإهلاك بضغطة واحدة")
    ...
    if pi_purchase_type == "آجل (على الحساب)":
        with st.expander("➕ إضافة مورد جديد بسرعة"):
            ...
            ns_open = ns3.number_input("رصيد افتتاحي", min_value=0.0, step=0.01, key="pi_new_supp_open")
    ...
    c1, c2 = st.columns(2)
    pi_date = c1.date_input("تاريخ الفاتورة", value=date.today(), key="pi_date")
    pi_cash_acc = None
    ...
    pi_tax_choice = c2.selectbox(
        "الضريبة", pi_tax_choice_options,
        format_func=lambda t: "بدون ضريبة" if t == "بدون ضريبة"
    ...
    st.markdown("### 📦 بنود المخزون (اختياري)")
    uom_opts_df = product_uom_options_df()
    uom_label_map = {}
    ...
    if uom_opts_df.empty:
        ...
        edited_inv = st.data_editor(
            ...
            column_config={
                "الصنف / الوحدة": st.column_config.SelectboxColumn("الصنف / الوحدة", options=inv_options, required=True),
                "الكمية": st.column_config.NumberColumn("الكمية", min_value=0.0, step=0.01),
    ...
    st.markdown("### 🏢 بنود أصول ثابتة (اختياري)")
    ...
    edited_assets = st.data_editor(
        ...
        column_config={
            "وصف الأصل": st.column_config.TextColumn("وصف الأصل"),
            "المبلغ": st.column_config.NumberColumn("المبلغ", min_value=0.0, step=0.01),
        },
        key="pi_asset_editor"
    )

    preview_inv_total = float((edited_inv["الكمية"] * edited_inv["تكلفة الوحدة"]).sum()) if not edited_inv.empty else 0.0
    preview_asset_total = float(edited_assets["المبلغ"].sum()) if not edited_assets.empty else 0.0
    pc1, pc2 = st.columns(2)
    pc1.metric("إجمالي بنود المخزون (معاينة)", f"{preview_inv_total:,.2f}")
    pc2.metric("إجمالي بنود الأصول الثابتة (معاينة)", f"{preview_asset_total:,.2f}")

    if st.button("✅ ترحيل فاتورة المشتريات", type="primary"):
        lines = []
        for _, r in edited_inv.iterrows():
            ...
            uom_id = uom_label_map[label]
            prod_id = int(uom_opts_df[uom_opts_df["uom_id"] == uom_id]["product_id"].values[0])
            lines.append({"line_type": "مخزون", "product_id": prod_id, "uom_id": uom_id,
                          "quantity": float(r["الكمية"]), "unit_cost": float(r["تكلفة الوحدة"])})
        for _, r in edited_assets.iterrows():
            if r["وصف الأصل"] and r["المبلغ"] > 0:
                lines.append({"line_type": "أصل_ثابت", "description": r["وصف الأصل"], "line_total": float(r["المبلغ"])})
